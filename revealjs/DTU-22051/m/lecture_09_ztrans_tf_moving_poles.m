...
path2pic = ['..' filesep 'pics' filesep 'lecture9-movie-zplane-tf-moving-poles' filesep];
...
zero_s = [-1/2 (increment*k); -1/2 -(increment*k)];
zero_s_Z = [-1/2+j*(increment*k); -1/2-j*(increment*k)];
poles = [0 0; 0 0];
poles_Z = [0; 0];

% get the impulse response
%hsos = zp2sos(zero_s_Z,poles_Z,1);
hsos = zp2sos(poles_Z,zero_s_Z,1);
...
ypos = sin(2*pi*f_all/fs);
    ...
    filename = [path2pic 'lecture9_zplane_tf_movingpoles_frame_' num2str(k) '.eps'];
